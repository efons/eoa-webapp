...
ui_db <- dashboardPage(
  ...
  dashboardSidebar(width=200,
    sidebarMenu(id="menu_items", selected ="water_quality",
      menuItem("Overview", tabName="overview", icon=icon("home")),
      menuItem("Water Quality", tabName = "water_quality", icon=icon("tint"), startExpanded = T,
      ...
      menuItem("Pollutants of Concern", tabName = "poc", icon=icon("exclamation")),
    ...
    tabItems(
      ...
      tabItem(tabName = "bio_data",
               ...
               fluidRow(
                  ...
                  box(width=12, status = "warning", 
                      div(style="font-weight:blod; color:orange; text-align:center",
                          fluidRow(h4("Creek Health Indicator"))),
              ...
              box(width=NULL, status='primary',
                  fluidRow(column(4,sliderInput(inputId = "wy", label=NULL, ticks=T,
                  ...
                  column(6, prettyCheckbox(inputId="show_radius", label="Show stressor variable as marker size?", 
                                       value=F, shape="round", animation="pulse", fill=F))))), 
              
              
              column(6,

              # Choose stressor variable
                     selectizeInput(tags$style(type='text/css', ".selectize-dropdown-content {max-height: 200px; }"),
                                 inputId="size_by",
              ...
              tabBox(id="output_tabs", width=12,
                     ...
                     tabPanel(title="Plots",id="overview_plots",div(style="overflow-y: scroll; height: 700px",
                     ...
                     tabPanel(title="Site-specific", id="site_info",
      ...
      tabItem(tabName = "con_wq", 
              h2("Continuous Water Quality & Continuous Temperature"),
              
              # Box for inputs 
             fluidRow(box(width=3, sliderInput(inputId = "wq_yr", label="Year:", 
                                       min=min(wq_vars_yr), max=max(wq_vars_yr), 
                                       value = c(min(wq_vars_yr),max(wq_vars_yr)), sep=''),
                           selectInput(inputId = "wq_ws", label="Watershed:", 
                                        choices=c(wq_vars_ws),selected=wq_vars_ws[1])
                           ),
             #box for map
              box(width=9,leafletOutput("map_wq"))),
              
              # Box for plots
              fluidRow(tabBox(width=12, height=800, id="wq_plots",
                  tabPanel(title="Continuous Temperature", id="con_temp_plot",
                  selectInput(inputId = "temp_param", label="Parameter:", 
                              choices=c("Continuous Temperature" = "ConTemp", "Daily Average"="avDayTemp", "Daily Max"="maxDayTemp", "Weekly Average" = "avWeek", "Weekly Maximum" = "maxWeek"), 
                              selected="avDayTemp"),
                  plotOutput("temp_timeseries_1"),
                  plotOutput("temp_timeseries_2")),
                  tabPanel(title="Seasonal Water Quality Parameters", id="wq_plot",
                           fluidRow(column(3,selectInput(inputId = "wq_param", 
                                       label = "Water Quality Parameter:", 
                                       choices=c("Temperature" = "temp_c", "pH"="ph" , "Specific Conductivity" = "sp_cond_us_cm", "Dissolved Oxygen" = "do_mg_l"),
                                       selected='ph')),
                           column(3,selectInput(inputId = "wq_season", 
                                       label = "Season:", 
                                       choices=c("Spring"="S", "Fall"="F"), selected="S"))),
                           fluidRow(column(6,plotOutput("wq_boxplot_1"),
      ...
      tabItem(tabName = "poc",
              ...
              # Box for inputs 
              fluidRow(box( selectInput(inputId="poc_contaminant",label="Contaminant:", choices=c("Mercury"="hg", "PCB"="pcb"), selected = "hg"),
             ...
             # Box for plots 
                   box(plotOutput("plot_poc_1", height="200px"))),
